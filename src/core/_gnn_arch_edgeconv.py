...
class EdgeConvMLP(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, ec_mlp_width, activation_function=ELU()):
        """
        Initialize the MLP.
        ...
        mlp_layers_dim_list = [mlp_layer_width, mlp_layer_width] # input-[1stlayer, ... ,?layer]-output
        # hidden_layer_1
...
class LightningEdgeConvModel(pl.LightningModule):
    ...
    """
    def __init__(self, in_channels, out_channels, hidden_channels, num_edge_conv, ec_mlp_width, lr_initial=2e-3, gamma_decay=2e-3, use_optimizer="adam", lr=1e-2, activation_function=ELU(), skip_connection_type="concat"):
        ...
        for i in range(self.L):
            ...
            self.convs.append(EdgeConv(nn=EdgeConvMLP(in_channels=ec_mlp_in_channels, out_channels=self.hidden_neurons, ec_mlp_width=self.ec_mlp_width, activation_function=self.activation_function), aggr="max"))
        ...
        # Add intermediate layers - current model has one hidden layer - this loop doesn't create any layer
